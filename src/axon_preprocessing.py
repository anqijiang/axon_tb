...
class AxonImaging:
    ...
    def plot_choose_rois(self, proc_dict=None, worst_n=10):
        ...
        for env in proc_dict:
            ...
            proc_dict[env]['roi'] = proc_dict[env]['roi'][:-selected_rois-1]

        self._save_dict(proc_dict)

        return proc_dict

    def combine_rois(self, proc_dict=None):
    ...
    def _group_rois(self, final_mat, corr_thresh=0.7, stopping_thresh_perc=10, min_best_score=0.4):
        ...
        overall_corr = np.corrcoef(final_mat[grouping_rois, :])
        n_max_cluster = sum(max_corr > corr_thresh)
        print(f'{self.animal} {self.day} # potential non-singleton ROIs: {n_max_cluster}')
        plt.hist(max_corr)
        plt.title('max corr per ROI')
        plt.show()

        cluster_number_vector = np.arange(np.max((int(len(overall_corr) / 100), 2)), n_max_cluster)
        silhouette_all = np.zeros((len(cluster_number_vector)))
        clusters_all = np.zeros((len(cluster_number_vector), sum(max_corr > corr_thresh)))
        stopping_thresh = int(np.ceil(n_max_cluster * stopping_thresh_perc/100))

        # Flatten the covariance matrices
        flattened_cov_matrices = overall_corr.reshape(len(overall_corr), -1)

        # Scale the flattened covariance matrices
        scaler = StandardScaler()
        flattened_cov_matrices_scaled = scaler.fit_transform(flattened_cov_matrices)
        ...
        if best_score < min_best_score:
            print(f'{self.animal} {self.day} no significant cluster groups found, consider grouping by correlation')
            return final_mat, {}

        print(f'{self.animal} {self.day} best K: {best_k}, best Silhouette: {best_score}')
        grouped_mat, group_map = group_columns(final_mat[grouping_rois, :], best_k, clusters_all[best_k], grouping_rois)
        combined_mat = np.vstack([grouped_mat, final_mat[~grouping_rois, :]])
